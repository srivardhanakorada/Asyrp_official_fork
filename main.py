...
def parse_args_and_config():
    ...
    if not isinstance(level, int): raise ValueError('level {} not supported'.format(args.verbose))
    ...
    if args.run_test: shutil.copy(args.sh_file_name, os.path.join(args.exp, f"{(args.sh_file_name).split('.')[0]}_test.sh"))
    elif args.style_transfer: shutil.copy(args.sh_file_name, os.path.join(args.exp, f"{(args.sh_file_name).split('.')[0]}_style_transfer.sh"))
    elif args.run_train: shutil.copy(args.sh_file_name, os.path.join(args.exp, f"{args.sh_file_name.split('.')[0]}_train.sh"))
    elif args.lpips: pass
    args.training_image_folder = os.path.join(args.exp, 'training_images')
    if not os.path.exists(args.training_image_folder): os.makedirs(args.training_image_folder)
    args.test_image_folder = os.path.join(args.exp, 'test_images', str(args.n_test_step))
    if not os.path.exists(args.test_image_folder): os.makedirs(args.test_image_folder)  
    args.image_folder = os.path.join(args.exp, 'image_samples')
    if not os.path.exists(args.image_folder): os.makedirs(args.image_folder)
    else:
        overwrite = False
        if args.ni: overwrite = True
        else:
            response = input("Image folder already exists. Overwrite? (Y/N)")
            if response.upper() == 'Y': overwrite = True
        if overwrite:
...
def main():
    ...
    try:
        ...
        elif args.lpips: runner.compute_lpips_distance()
